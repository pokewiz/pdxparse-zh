...
instance RenderMessage Script ScriptMessage where
    renderMessage _ (useEnglish -> True) msg = case msg of
        ...
        MsgSignWestphalia
            -> "从现在起，信仰任何基督教派的 {{icon|hre}} 神圣罗马帝国皇帝、选帝侯及诸侯们都不再互相视为异端。"
        ...
        MsgMayExplore
            -> "{{icon|may explore|28px}} 可以征召探险家和征服者。探险家可以探索海外省份。"
        ...
        MsgRankKingdom
            -> "政府等级至少为 {{icon|kingdom}} 王国或 {{icon|empire}} 帝国<!-- 不是 {{icon|duchy}} 公国 -->"
        MsgRankEmpire
        ...
        MsgIsFreeOrTributaryTrigger {scriptMessageYn = _yn}
            -> mconcat
                ...
                , toMessage (ifThenElseT _yn "" " ''不''")
                , "是独立国家"
                , toMessage (ifThenElseT _yn "或" "''也不是''")
                , "是朝贡国"
                ]
